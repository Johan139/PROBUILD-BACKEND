...
namespace BuildigBackend.Services
{
    public class JobNotificationService
    {
        ...
        public async Task NotifyUsersAboutNewJob(JobModel job)
        {
            ...
            //string templatePath = Path.Combine(Directory.GetCurrentDirectory(), "..", "BUILDIG-BACKEND", "BuildigBackend", "EmailTemplates", "NewJobNotification.html");
        ...
        .Replace("{{JobDetailsLink}}", $"https://app.buildig.ai/jobs/{job.Id}")
        .Replace("{{UnsubscribeLink}}",
            $"https://app.buildig.ai/subscription/unsubscribe?email={user.Email}")
                };
            ...
            foreach (var externalUser in filteredExternalUsers)
            {
                ...
                )
                {
                    var email = new EmailTemplate
                    {
                        ...
                        .Replace(
                            "{{JobDetailsLink}}",
                            $"https://app.buildig.ai/register?email={externalUser.email}"
