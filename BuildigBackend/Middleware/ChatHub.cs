...
namespace BuildigBackend.Middleware
{
    [Authorize]
    public class ChatHub : Hub
    {
        ...
        public override Task OnDisconnectedAsync(Exception? exception)
        {
            if (exception != null)
                _logger.LogWarning(
                    exception,
                    "Client disconnected due to error: {ConnectionId}",
        ...
        public async Task JoinConversationGroup(string conversationId)
        {
            _logger.LogInformation(
                "JoinConversationGroup called for conversation {ConversationId} by connection {ConnectionId}",
                conversationId,
                Context.ConnectionId
            );
            ...
            _logger.LogInformation(
                "Context.UserIdentifier (for comparison): {UserIdentifier}",
            ...
            {
                _logger.LogWarning(
                    "Unable to extract user ID from claims for conversation {ConversationId}",
                    conversationId
                );
                return;
            }
            ...
            {
                _logger.LogWarning(
                    "Attempt to join a non-existent conversation {ConversationId}",
                    conversationId
                );
                return;
            }
            ...
            {
                _logger.LogWarning(
                    "User {UserId} attempted to join conversation {ConversationId} without permission. Expected: {ExpectedUserId}",
                    userId,
                    conversationId,
                    conversation.UserId
                );
            ...
            _logger.LogInformation(
                "User {UserId} successfully joined conversation group {ConversationId}",
                userId,
                conversationId
            );
        }
