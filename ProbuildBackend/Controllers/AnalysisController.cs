...
namespace ProbuildBackend.Controllers
{
    ...
    public class AnalysisController : ControllerBase
    {
        ...
        public AnalysisController(
            IWalkthroughService walkthroughService,
        ...
        public async Task<IActionResult> StartWalkthrough(
            [FromBody] StartWalkthroughRequestDto request
        )
        {
            ...
            var initialAnalysisPrompt = await _promptManager.GetPromptAsync(
                "",
                initialAnalysisPromptKey
            );
            var systemPersonaPrompt = await _promptManager.GetPromptAsync(
                "",
                systemPersonaPromptKey
            );

            var (initialAiResponse, conversationId) =
            ...
            var walkthroughSession = await _walkthroughService.StartSessionAsync(
                jobId,
                userId,
                conversationId,
                initialAiResponse,
        ...
        public async Task<IActionResult> GetNextStep(
            Guid sessionId,
            [FromQuery] bool applyCostOptimisation = false
        )
        {
            var result = await _walkthroughService.GetNextStepAsync(
                sessionId,
                applyCostOptimisation
            );
            return Ok(result);
        }
