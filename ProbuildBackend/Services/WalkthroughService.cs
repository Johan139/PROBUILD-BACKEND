...
namespace ProbuildBackend.Services
{
    public class WalkthroughService : IWalkthroughService
    {
        ...
        public async Task<WalkthroughSession> StartSessionAsync(
            int jobId,
            string userId,
        ...
        private List<string> GetPromptSequence(string analysisType, List<string> selectedPrompts)
        {
            switch (analysisType)
            {
                ...
                case "sequential":
                    ...
                    {
                        "prompt-01-site-logistics.txt",
                        "prompt-02-quality-management.txt",
                        "prompt-03-demolition.txt",
                        "prompt-04-groundwork.txt",
                        "prompt-05-framing.txt",
                        "prompt-06-roofing.txt",
                        "prompt-07-exterior.txt",
                        "prompt-08-electrical.txt",
                        "prompt-09-plumbing.txt",
                        "prompt-10-hvac.txt",
                        "prompt-11-fire-protection.txt",
                        "prompt-12-insulation.txt",
                        "prompt-13-drywall.txt",
                        "prompt-14-painting.txt",
                        "prompt-15-trim.txt",
                        "prompt-16-kitchen-bath.txt",
                        "prompt-17-flooring.txt",
                        "prompt-18-exterior-flatwork.txt",
                        "prompt-19-cleaning.txt",
                        "prompt-20-risk-analyst.txt",
                        "prompt-21-timeline.txt",
                        "prompt-22-general-conditions.txt",
                        "prompt-23-procurement.txt",
                        "prompt-24-daily-construction-plan.txt",
                        "prompt-25-cost-breakdowns.txt",
                        "prompt-26-value-engineering.txt",
                        "prompt-27-environmental-lifecycle.txt",
                ...
                case "Renovation":
                    return new List<string>
                    {
                        "renovation-01-demolition.txt",
                        "renovation-02-structural-alterations.txt",
                        "renovation-03-rough-in-mep.txt",
                        "renovation-04-insulation-drywall.txt",
                        "renovation-05-interior-finishes.txt",
                        "renovation-06-fixtures-fittings-equipment.txt",
                        "renovation-07-cost-breakdown-summary.txt",
                        "renovation-08-project-timeline.txt",
                        "renovation-09-environmental-impact.txt",
                        "renovation-10-final-review-rebuttal.txt",
                        "executive-summary-prompt.txt",
                    };
                default:
                    throw new ArgumentException(
                        "Invalid analysis type provided.",
                        nameof(analysisType)
                    );
            }
        }

        public async Task<WalkthroughStep> GetNextStepAsync(
            Guid sessionId,
            bool applyCostOptimisation = false
        )
        {
            ...
            var promptNames = System.Text.Json.JsonSerializer.Deserialize<List<string>>(
                session.PromptSequenceJson
            );

            var lastStep = await _context
                .WalkthroughSteps.Where(s => s.SessionId == sessionId)
            ...
            {
                var costOptimisationPrompt = await _promptManager.GetPromptAsync(
                    "prompts",
                    "prompt-26-value-engineering.txt"
                );
            ...
            var (aiResponse, conversationId) = await _aiService.ContinueConversationAsync(
                lastStep.ConversationId,
                null,
                nextPrompt,
                null
            );
        ...
        public async Task<WalkthroughStep> RerunStepAsync(
            Guid sessionId,
            int stepIndex,
            RerunRequestDto data
        )
        {
            var currentStep = await _context.WalkthroughSteps.FirstOrDefaultAsync(s =>
                s.SessionId == sessionId && s.StepIndex == stepIndex
            );
            ...
            var originalPromptText = await _promptManager.GetPromptAsync(
                "prompts",
                currentStep.PromptKey
            );

            var metaPrompt = new System.Text.StringBuilder();
            metaPrompt.AppendLine(
                "You are an AI assistant tasked with refining a construction analysis based on user feedback."
            );
            ...
            metaPrompt.AppendLine(
                "1. Integrate all of the user's inline edits from their 'EDITED VERSION' exactly as they appear."
            );
            metaPrompt.AppendLine("2. Address the points raised in the 'GENERAL COMMENTS'.");
            metaPrompt.AppendLine(
                "3. Recalculate or adjust any other parts of the original response that are impacted by the user's changes, but do not alter the locked sections."
            );
            metaPrompt.AppendLine(
                "4. Present the final output as a complete, clean response without mentioning this feedback process."
            );

            var (newAiResponse, _) = await _aiService.ContinueConversationAsync(
        ...
        public async Task<WalkthroughSession> GetSessionAsync(Guid sessionId)
        {
            var session = await _context
                .WalkthroughSessions.Include(s => s.Steps.OrderBy(step => step.StepIndex))
